...
<style>
  ...
  /* Kill Streamlit top padding — masthead must sit flush */
  ...
  .masthead-sub a { color: #E8C96A !important; }
  .masthead-badge {
    font-family: var(--display) !important;
    font-size: 14px;
...
def codex_card(regime, cer, matrix_ascii, use_cases, pairs, risk, risk_class):
    ...
    pairs_html   = " ".join(f'<span class="codex-badge {badge_class}">{p}</span>' for p in pairs)
...
def main():
    ...
    # ── MASTHEAD — rendered as single HTML block, no st.columns ───────────────
    if ted_b64:
        ted_html = f"""
        <div class="ted-zone">
          <div class="speech-bubble">Ted said so.</div>
          <img class="ted-img" src="data:image/jpeg;base64,{ted_b64}" alt="Ted">
          <div class="ted-caption">TED · STOOP TABBY<br>Certified this pipeline.<br>HuggingFace could not see Ted.</div>
        </div>"""
    else:
        ted_html = """
        <div class="ted-zone">
          <div class="speech-bubble">Ted said so.</div>
          <div style="width:110px;height:110px;border:4px solid #F5EDD6;margin-top:8px;
               display:flex;align-items:center;justify-content:center;">
            <span style="font-family:'Bangers',cursive;font-size:32px;color:#F5EDD6;">TED</span>
          </div>
          <div class="ted-caption">TED · STOOP TABBY</div>
        </div>"""
    ...
    <div class="masthead">
      <div class="masthead-left">
        ...
        <div class="masthead-sub">
          ...
          READ THE CONFUSION MATRIX &nbsp;·&nbsp; KNOW WHAT YOUR MODEL LEARNED<br>
    ...
    st.markdown(
        "Tell us what's going wrong — plain language, letter patterns, "
        "or upload your data below. The ring decodes it."
    )
    ...
    if diagnose_clicked:
        if not user_input.strip():
            ...
            with r2:
                ...
                st.markdown(f'<span class="codex-risk {risk_class}">{risk}</span>',
                            unsafe_allow_html=True)
        ...
        if uploaded is not None:
            try:
                ...
                if dtype == "inference_log":
                    ...
                    st.markdown(f"**Matrix:** {summary['shape'][0]}×{summary['shape'][1]}")
                    if api_key:
                        matrix_desc = (
    ...
    with c2:
        if ted_b64:
            st.markdown(f"""
            <div style="border:4px solid var(--ink);box-shadow:4px 4px 0 var(--ink);overflow:hidden;">
              <img src="data:image/jpeg;base64,{ted_b64}"
                   style="width:100%;display:block;" alt="Ted">
              <div style="font-family:'Share Tech Mono',monospace;font-size:9px;
                          color:#7A6540;text-align:center;padding:5px;
                          background:#E8D9B0;border-top:2px solid #1A1209;">
                Gemma 4 saw Ted via Ollama.<br>
